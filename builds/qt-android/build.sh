...
##
# Make sure jansson is built and copy the prefix

(android_build_verify_so "libjansson.so" &> /dev/null) || {
    # Use a default value assuming the jansson project sits alongside this one
    test -z "$JANSSON_ROOT" && JANSSON_ROOT="$(cd ../../../jansson && pwd)"
    
    if [ ! -d "$JANSSON_ROOT" ]; then
        echo "The JANSSON_ROOT directory does not exist"
        echo "  ${JANSSON_ROOT}"
        exit 1
    fi
    
    (bash ${JANSSON_ROOT}/builds/qt-android/build.sh) || exit 1
    UPSTREAM_PREFIX=${JANSSON_ROOT}/builds/qt-android/prefix/${TOOLCHAIN_NAME}
    cp -r ${UPSTREAM_PREFIX}/* ${ANDROID_BUILD_PREFIX}
}

##
# Build zgtask from local source

(android_build_verify_so "libzgtask.so" "libczmq.so" "libjansson.so" &> /dev/null) || {
    rm -rf "${cache}/zgtask"
    (cp -r ../.. "${cache}/zgtask" && cd "${cache}/zgtask" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/zgtask" && ./autogen.sh \
...
android_build_verify_so "libjansson.so"
android_build_verify_so "libzgtask.so" "libczmq.so" "libjansson.so"
